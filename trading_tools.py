...
def prepare_master_dataframe(start_date, ticker, download_data):
    ...
    if df_1d.empty or df_1h.empty or df_4h.empty or df_30m.empty or df_15m.empty or df_5m.empty or df_1m.empty:
        ...
        # Возвращаем пустые DataFrame'ы, чтобы избежать падения
    ...
    features_4h['atr_14_4h'] = df_4h.ta.atr(length=14)  # Важно для trend_strength

    # 4. Объединение в final_df (мастер-фрейм 1H)
    final_df = df_1h.copy()
    ...
    print("✅ Мастер-DataFrame и LTF данные успешно подготовлены.")
    return final_df, df_30m, df_15m, df_5m, df_1m

    # ==============================================================
    # features_4h = pd.DataFrame(index=df_4h.index)
    # # Рассчитываем EMA (как и раньше)
    # features_4h['ema_50_4h'] = df_4h.ta.ema(length=50, append=False)
    # # ДОБАВЛЯЕМ РАСЧЕТ ADX НА 4Ч ТФ
    # adx_4h = df_4h.ta.adx(length=14, append=False)
    # # Берем только сам ADX и переименовываем, чтобы не было конфликтов
    # if adx_4h is not None and 'ADX_14' in adx_4h.columns:
    #     features_4h['adx_14_4h'] = adx_4h['ADX_14']
    #
    #
    ...
    # final_df.fillna(method='ffill', inplace=True)
    # final_df.dropna(inplace=True)
    # print("✅ Данные со всех таймфреймов успешно объединены.")
...
class FeatureEngineSMC:
    ...
    STATIC_FEATURES_LONG = [
        'trend_strength_4h',
        'dist_to_static_pdl_atr',  # Ключевой признак для лонга
        'bullish_rejection_power',  # Ключевой признак для лонга
        ...
        # Добавь сюда ADX, RSI, F&G и т.д., если хочешь их использовать
    ]
    ...
    STATIC_FEATURES_SHORT = [
        ...
        'dist_to_static_pdh_atr',  # ОСТАВЛЯЕМ. (Полезно знать, если Asia High == PDH)
        'dist_to_static_pdl_atr',  # ОСТАВЛЯЕМ. (Контекст)
        # --- 4. КОНТЕКСТ СВЕЧИ ---
        'bearish_rejection_power',  # Форма 1Ч свечи (важно для sweep)
    ...
    #         'dynamic_leg_vol',  # <-- из Шага 2
    #         'dynamic_candle_vol',  # <-- из Шага 2
    #         'dynamic_vol_ratio',  # <-- из Шага 2
    #         'structure_state'
    #     ]
    ...
    def _label_target_short(self, look_forward_bars=6):
        """
        [НОВАЯ ЗАДАЧА v11 - Flexible Sessions Regression]
        Задача: Предсказать РАЗМЕР движения после ГИБКОГО сессионного паттерна.
        - В Лондоне ищем свип Азии.
        - В Нью-Йорке ищем свип максимума дня (Азия или Лондон).
        """
        print("Разметка РЕГРЕССИОННОЙ Цели (v11 - Гибкие Сессии)...")
        df = self.df

        # --- 1. Находим максимумы и диапазоны КАЖДОЙ сессии ---
        is_asian = (df['is_asian_session'] == 1)
        ...
        # --- 2. Определяем, КАКОЙ уровень и диапазон являются целями для КАЖДОЙ свечи ---
        target_liquidity_level = pd.Series(float('nan'), index=df.index)
        normalization_range = pd.Series(float('nan'), index=df.index)

        # Для свечей в Лондонской КЗ, цель - это максимум Азии
        ...
        # Для свечей в Нью-Йоркской КЗ, цель - это МАКСИМУМ из хая Азии и хая Лондона
        is_newyork_kz = (df['is_newyork_killzone'] == 1)
        # .shift(1) нужен, чтобы данные на момент NY KZ были историческими
        ...
        # Для нормализации используем больший из диапазонов
        ...
        # --- 3. Находим свечи, которые соответствуют нашему ГИБКОМУ паттерну ---
        ...
        # --- 4. РАССЧИТЫВАЕМ РЕЗУЛЬТАТ для каждого кандидата ---
        ...
        print(f"Разметка завершена. Найдено {len(candidate_candles)} гибких сессионных событий для анализа.")
    ...
    def run(self, model_type: str, create_target: bool = False):
        ...
        if create_target:
            ...
            if model_type == 'LONG':
                ...
                self._label_target_short()  # <-- Теперь эта функция сработает, т.к. колонка 'dynamic_vol_ratio' существует

        # --- ЭТАП 6: Финальное формирование X и y ---
        print("Формирование итоговой матрицы признаков...")
